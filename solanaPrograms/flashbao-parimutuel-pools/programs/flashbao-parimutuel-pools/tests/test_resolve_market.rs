...
fn test_resolve_market() {
    ...
    let outcome_count = 3;
    let market_id = [10_u8; 32];
    let metadata_uri = String::from("https://metadata.flashbao.example/markets/10.json");
    let metadata_hash = [12_u8; 32];
    ...
    let winning_outcome: u16 = 1;
    let nonce: u64 = 12345;
    let timestamp: i64 = 1_600_000_000;

    let resolve_market_instruction = Instruction::new_with_bytes(
        program_id,
    ...
    assert_eq!(
        market_state.status,
        flashbao_parimutuel_pools::state::MarketStatus::Resolved
    );
    assert_eq!(market_state.winning_outcome, Some(winning_outcome));
